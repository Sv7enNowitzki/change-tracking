...
const _getRootEntityPathVals = function (txContext, entity, entityKey) {
  ...
  if (txContext.event === "CREATE") {
    ...
    if (
      txContext.event === "DELETE" &&
      !entityIDs.find((x) => x === entityKey)
    ) {
...
const _formatAssociationContext = async function (changes) {
  for (const change of changes) {
    const a =
      cds.model.definitions[change.serviceEntity].elements[change.attribute];
    ...
    const fromObjId = await getObjectId(a.target, semkeys, {
      curObjFromDbQuery: from || undefined
    }); // Note: ... || undefined is important for subsequent object destructuring with defaults
    if (fromObjId) change.valueChangedFrom = fromObjId;

    const toObjId = await getObjectId(a.target, semkeys, {
      curObjFromDbQuery: to || undefined
    }); // Note: ... || undefined is important for subsequent object destructuring with defaults
...
const _formatCompositionValue = function (
  ...
  if (curChange.modification === "update") {
    ...
    const diffAttrs = rowOldAttrs.filter(
      (attr) => managedAttrs.indexOf(attr) === -1
    );
    if (!diffAttrs.length) {
      return;
    }
...
const _getObjectIdByPath = async function (
  ...
  const curObjFromReqData = getCurObjFromReqData(
    reqData,
    nodePathVal,
    serviceEntityPath
  );
...
function _trackedChanges4(srv, target, diff) {
  const template = getTemplate("change-logging", srv, target, {
    pick: (e) => e["@changelog"]
  });
...
const _prepareChangeLogForComposition = async function (
  entity,
  entityKey,
  changes,
  req
) {
  const rootEntityPathVals = _getRootEntityPathVals(
    req.context,
    entity,
    entityKey
  );
...
async function track_changes(req) {
  ...
  // cds.transaction(req).context.event = "delete" undefined
  if (req.event === "DELETE") {
  ...
  if (isComposition && !isDraftEnabled) {
    [target, entityKey] = await _prepareChangeLogForComposition(
      target,
      entityKey,
      changes,
      this
