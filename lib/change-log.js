...
function _trackedChanges4 (srv, target, diff, flag) {
  ...
  if (flag) {
    ...
    diff._path = _getRootEntityPathVals(srv.context, target.name, diff.ID).join("/")
  }
...
async function track_changes (req) {
  ...
  if (typeof req.target.actions === "object") {
    for (const item of Object.values(req.target.actions)) {
        if (item?.kind === "action" && !item.returns) {
            flag = false;
        }
    }
    if (!flag) {
        rootEntityPathVals = _getRootEntityPathVals(req.context, target.name, entityKey);
        rootEntity = getNameFromPathVal(rootEntityPathVals[0]);
        rootEntityID = getUUIDFromPathVal(rootEntityPathVals[0]);
    }
  }
  ...
  //   "DELETE",
  //   undefined
  // ].some((event) => event === req.context.event)

  // if (!flag) {
  //   rootEntityPathVals = _getRootEntityPathVals(
  //     req.context,
  //     target.name,
  //     entityKey
  //   );
  ...
  //   rootEntityID = getUUIDFromPathVal(rootEntityPathVals[0]);
  // }
  let changes = _trackedChanges4(this, target, diff, flag)
  if (!changes) return
  ...
  const dbEntity = getDBEntity(flag ? target : rootEntity)
  await INSERT.into("sap.changelog.ChangeLog").entries({
    entity: dbEntity.name,
    entityKey: flag ? entityKey : rootEntityID,
    serviceEntity: flag ? target.name : rootEntity,
