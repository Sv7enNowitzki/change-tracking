...
const getCurObjFromDbQuery = async function (
  entityName,
  queryVal,
  /**optional*/ queryKey = "ID"
) {
  if (!queryVal) return {};
...
async function getObjectId(entityName, fields, curObj) {
  let all = [],
    { curObjFromReqData: req_data = {}, curObjFromDbQuery: db_data = {} } =
      curObj;
  let entity = cds.model.definitions[entityName];
  if (!fields?.length)
    fields = entity["@changelog"]?.map?.((k) => k["="] || k) || [];
        ...
        if (IDval)
          try {
            ...
            const isDraftEnabled = !!assoc._target.drafts,
              isComposition = hasComposition(assoc._target, current);
            ...
            LOG.error(
              "Failed to generate object Id for an association entity.",
              e
            );
            throw new Error(
              "Failed to generate object Id for an association entity.",
